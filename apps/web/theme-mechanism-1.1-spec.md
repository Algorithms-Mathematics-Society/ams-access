...
| `home/page.tsx` (×2)      | `localStorage.setItem(STORAGE_KEYS.THEME,"dark")` **and** `document.documentElement.className = "dark"` | **Remove BOTH** (discovered during impl — the spec originally listed only the localStorage write). The `className =` assignment was a second competing writer of the canonical `html.className` that `apply()` now owns, AND an assignment (not `classList.add`) that obliterated the layout's `inter`/`jetbrains` font-variable classes — a pre-existing latent font-degradation bug. Leave `setTheme("dark")` — home's internal **JS-prop theme STATE** stays dark until 2a; that is the only home theming deferred. |
...
**Consequence of removing home's `className` write — a 1.2-gating item (recorded).** With the force-pin
gone, home's `html.className` follows the canonical resolved theme. Home's bulk content stays dark (its
JS-prop ternaries are pinned `theme="dark"` → `getThemeColors`), but the **thin set of home sites that
consume `var(--theme-*)`/`var(--surface-*)` directly** (≈25 inline usages in `home/**` + classes
`.home-nav-btn`, `.ic-btn-violet`, `.route-loading-shell--home`) **would half-flip to light** for a
system-light user on unmigrated home. This is exactly what **1.2's `.theme-dark-locked`** pins. Home is
post-auth (1.1 proof never reaches it) and the branch does not release until 1.2 — but this is a **hard
1.2 requirement: home must carry the dark-lock before any release, or it ships half-themed.**
